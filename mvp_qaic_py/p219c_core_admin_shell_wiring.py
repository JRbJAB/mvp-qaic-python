...
def _candidate_rank(candidate: dict[str, Any]) -> tuple[int, int, int, int, str]:
    ...
    return (
        layer_bonus + menu_bonus + route_bonus + nicegui_bonus + score,
        layer_bonus,
        menu_bonus,
        route_bonus,
...
def select_primary_nicegui_shell(core_registry: dict[str, Any]) -> dict[str, Any]:
    ...
    selected = sorted(candidates, key=_candidate_rank, reverse=True)[0]
    return {
        "STATUS": "OK_P219C_PRIMARY_NICEGUI_SHELL_SELECTED",
        "selected": selected,
        "reason": "BEST_SCORE_LAYER_MENU_ROUTE_SELECTION",
    }
...
def build_core_admin_shell_payload(
    ...
    *,
    ...
    if selection["STATUS"].startswith("REVIEW_"):
        status = "REVIEW_P219C_CORE_ADMIN_SHELL_NEEDS_EXISTING_UI_CONFIRMATION"
    ...
    return {
        ...
        "core_registry": core,
        "shell_strategy": {
            "mode": "ADAPTER_FIRST_NO_DESTRUCTIVE_PATCH",
            "next_patch": "wire render_core_admin_shell_nicegui into selected shell/menu",
            "selected_shell_path": (selection.get("selected") or {}).get("path"),
            "reason": (
                "Conserve existing cockpit shell; add admin sections through a safe adapter layer."
            ),
        },
        "local_only": True,
        "review_only": True,
        ...
        "order": False,
        "sizing": False,
        "recommended_next": "P219D_PATCH_SELECTED_NICEGUI_SHELL_MENU_WITH_CORE_ADMIN",
    }
...
def build_core_admin_shell_html(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "<h2>Shell NiceGUI sélectionné</h2>",
            "<ul>",
...
def render_core_admin_shell_nicegui(ui: Any, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    markdown = (
        ...
        "- Mode: adapter-first, no destructive patch\n"
    )
...
def build_shell_wiring_markdown(payload: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "## Next",
            "",
            "P219D: patch selected NiceGUI shell/menu with this core admin navigation.",
        ]
    )
    return "\n".join(lines)
