& {
  try {
    ...
    if (-not $run) {
      throw "STOP: aucun runtime P_REFLEX_06C trouve"
    }
    ...
    robocopy (Join-Path $SRC "mvp_qaic_reflex_ui") (Join-Path $RUN_DIR "mvp_qaic_reflex_ui") /E /PURGE /NFL /NDL /NJH /NJS /NP /XF "desktop.ini" | Out-Host
    if ($LASTEXITCODE -gt 7) { throw "STOP: robocopy UI failed" }
    $global:LASTEXITCODE = 0

    robocopy (Join-Path $SRC "assets") (Join-Path $RUN_DIR "assets") /E /NFL /NDL /NJH /NJS /NP /XF "desktop.ini" | Out-Host
    if ($LASTEXITCODE -gt 7) { throw "STOP: robocopy assets failed" }
    $global:LASTEXITCODE = 0
    ...
    switch ($nodeArch) {
      ...
      default { throw "STOP: architecture Node non geree: $nodeArch" }
    }
