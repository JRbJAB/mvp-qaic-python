& {
  try {
    ...
    Copy-Item `
      -LiteralPath (Join-Path $SRC "rxconfig.py") `
      -Destination (Join-Path $RUN_DIR "rxconfig.py") `
      -Force

    robocopy `
      (Join-Path $SRC "mvp_qaic_reflex_ui") `
      (Join-Path $RUN_DIR "mvp_qaic_reflex_ui") `
      /E /NFL /NDL /NJH /NJS /NP /XF "desktop.ini" |
      Out-Host

    if ($LASTEXITCODE -gt 7) {
      throw "STOP: robocopy UI failed"
    }
    ...
    if ($LASTEXITCODE -ne 0) {
      ...
      if ($LASTEXITCODE -ne 0) {
        throw "STOP: npm install binding failed"
      }
    }
    ...
    Write-Host "============================================================"
    Write-Host "P_REFLEX_10 LOCAL PRIVATE PREVIEW"
    ...
    Write-Host "STATUS=FAILED_P_REFLEX_10_LOCAL_PREVIEW"
