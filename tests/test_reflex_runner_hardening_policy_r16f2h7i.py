...
RULE_SUBSTRINGS = ['timeout dur sur chaque sous-commande native', 'docker run sans image preflight', 'docker run sans port preflight', 'docker exec si container non running', 'Copy-Item fichier par fichier', 'ZIP sans self-check de structure', 'rapport transitoire dans docs/']
